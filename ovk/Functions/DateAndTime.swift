...
func convertTimestampToPostTime(timestamp: Int) -> String {
    let currentTime = Int(Date().timeIntervalSince1970)
    let timeDifference = (currentTime - timestamp) / 60
    
    if timeDifference < 60 {
        return "\(timeDifference) \(getLocalizedString(key: "minutes_ago"))"
    }
        ...
        return "\(timeDifference/60) \(getLocalizedString(key: "hour"))\(hour_a)\(getLocalizedString(key: "ago"))"
    }
    else {
        ...
        return "\(formattedDate_1) \(getLocalizedString(key: "at")) \(formattedDate_2)"
    }
}
